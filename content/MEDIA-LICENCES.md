...
Every row points at **`tests/bin/generate-demo-media.py`**, and that is a real file
in this project's repository. Run it and it re-derives all thirty-nine files
from the entity exports under `content/` and compares them, byte for byte,
...
exact bytes as literal text. Both halves are named here rather than left to be
inferred, because "generated" means two different things for them.
