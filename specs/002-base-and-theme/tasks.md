...
| T-807 ✓ | · | ✅ **Signed 2026-08-26. CLOSURE REPORT — unit 002, base and theme. Every number below is a command's output or an API read; anything not measured is named as not measured, which is this row's actual criterion.**  **① THE BLOCKER IS DISCHARGED, AND IT IS OBSERVED RATHER THAN INFERRED.** [andres] published `drupal/agora_theme 1.0.1`. The `Drupal CMS` job, job `11779330` on pipeline `936774`, printed: `Locking drupal/agora_theme (1.0.1)` · `Downloading` · `Installing drupal/agora_theme (1.0.1): Extracting archive` · `OK (1 test, 1 assertion)`. Corroborated independently before the pipeline ran: `packages.drupal.org` lists `1.0.1` with a dist URL, and the packaged zip was downloaded and opened — **9 templates including `views-view-table.html.twig`** (`1.0.0` had 8 and not that one) plus the four `block.block.*` placements. **From now on every clean install receives the fix.** T-806's condition 2 moves from asserted to measured, which is the distinction this unit spent two days learning to make.  **② SITE TEMPLATE — pipeline `936774`, ref `1.x`, commit `108b51e`, job list read from the API.** 9 jobs · 9 `success` · 9 `allow_failure: false` · 0 named exceptions: `Drupal CMS`, `agora-invariants`, `composer`, `composer-lint`, `cspell`, `eslint`, `phpcs`, `phpstan`, `phpunit`. · **PHPUnit `OK (16 tests, 1717 assertions)`**, with `--fail-on-empty-test-suite` visible in the executed command line and `_PHPUNIT_CONCURRENT=0`. · **`agora-invariants`: 61 checks · 0 failures and 43 checks · 0 failures, 13 invariants**, each with its denominator: `no-secrets` **190 files** (config/ 102 · content/ 3 · 1 suppressed · 1 self-excluded · **2 binaries opened, 0 EXIF, 0 XMP**) · `no-patches` 1 file · `no-unstable-deps` 1 file · `sbom-check` **10 queried** across 2 files · `no-code-in-template` **118 packaged entries / 192 working files / 0 `*.info.yml` / 0 forbidden dirs** · `no-boilerplate` **228 scanned, 9 deny terms** · `no-ci-allow-dev` 192 files, 11 mentions, **0 definitions** · `no-blind-phpunit` 2 CI files, 2 invocations, **2 guarded, 0 unguarded** · `cited-tasks-exist` **89 citations / 49 distinct / 155 definitions / 144 distinct** · `config-inventory` **102 config objects, 0 nested, 0 zero-byte** · `media-licence` **3 content entries working / 3 packaged**, 0 binaries, printed `NOT YET LOAD-BEARING` · `no-real-people` **4 files, 0 roster rows, 12 deny terms, 7 shape patterns**. All 0 findings. · `spellcheck` (local replica) **187 files checked, 0 issues**.  **③ THEME — pipeline `936594`, ref `1.x`, commit `8979785`.** 9 jobs · 9 `success` · 9 blocking: `agora-invariants`, `composer`, `composer-lint`, `cspell`, `eslint`, `nightwatch`, `phpcs`, `phpstan`, `stylelint`. ⚠️ **It is NOT the same nine as the template's, and the count conceals what the names reveal (I-061).** Three differ, and the one that matters is stated as a gap rather than left to be inferred from a blank row: **the theme runs no `phpunit` job because it has NO PHP tests at all.** Owner [ejecutor], target unit 006. · **`nightwatch`: 6 pages scanned · 89-89 axe rules run per page · 0 violations · `heading-order` reported on 6 of 6 pages · 297 assertions across 11 tests.** ⚠️ Read from pipeline **`936572`**, commit `51cd4a3` — the last pipeline whose commit changed test content; `936594` is two doc/mode commits later and its job list is the one quoted above. Both are stated rather than merged. That job has been **seen to fail** on a real missing `alt` (pipeline `935776`), so its green is a measurement, not an absence. · **`gate-a-theme.sh`: 30 checks · 0 failures** — `no-secrets` **52 files** (config/ 4 · content/ **absent (0 files)** · **4 binaries opened, 0 EXIF, 0 XMP**) · `no-patches` 1 · `no-blind-phpunit` 1 file, 1 invocation, 1 guarded · `no-unstable-deps` 1 file, **0 require, 0 require-dev — vacuous, and it prints the number** · `shared-invariants` 13 files swept, **8 records, 5 invariant, 5 local-only** · `contrast-check` 5 files, **18 tokens, 38 pairs, 0 below threshold** · `no-colour-literals` **12 files** · `identity-strings` 3 identity files, **33 packaged files naming the product, 30 declared, 1 root `*.info.yml`**. All 0 findings. · `spellcheck` **45 files, 0 issues**.  **④ TASK COUNT.** `grep -c '^| T-[5-8][0-9][0-9] ' specs/002-base-and-theme/tasks.md` → **40**, against a budget of **38**. **Headroom −2**, reported against 38 rather than re-based, per D-031. ⚠️ `plan.md` said **34** in five places including this unit's own exit criterion until T-806's audit caught it — D-026 raised it to 38 and D-031 signed 40-against-38 — and it is amended, not edited.  **⑤ WHAT THE GREENS DO NOT COVER. Stated, not omitted — this is the row's criterion.** · **axe and Nightwatch have never run on this host** — no chromedriver, and `test-site.php` needs `drupal/core-dev`. CI is the sole oracle for every axe and assertion figure above. · **`phpcs`, `phpstan`, `eslint` and `stylelint` cannot run here either**, and none of them prints a denominator at all (I-045). · ⚠️ **Every CI *log* figure in this report is a TRANSCRIPTION, not something a third party can re-read.** GitLab job traces are not public for these projects (`/jobs/<id>/trace` → `401`). The job **lists** were re-read from the API by the auditor and by me; the **logs** were not, and a Drupal.org reviewer could not check them today. **Owner [andres], target unit 006** — enabling public pipelines on both projects is one setting each. · **Playwright and visual regression run NOWHERE.** T-804 is deferred to unit 003 with its prerequisite named; D-009(d) makes it informative and never a gate. · **macOS is NOT CERTIFIED** (T-803). Every local invariant count above is Windows-only. · **`content/` holds 3 files** — a blank Canvas page and two manifests. There is no demo content until unit 003, so the theme's axe green is over **fixtures**, not over the product.  **⑥ FOUR 🟡 CARRIED, each with an owner and a unit** — `no-secrets` upstream drift (closed early instead), `watch-gate`'s floor-versus-names (closed early instead), public job logs [andres]/006, and whether `drupal/site_template_helper` stays [andres]/006.  **🔒 HOLD — gate B needs [andres], and the gate cannot make these two judgements:** (a) is the **content model** right for Spanish transparency law — six bundles against the eight categories of Ley 19/2013, with the art. 8.1.a) derived statistic carried to unit 003; (b) is the **theme institutionally sober** enough for a public body. Everything a machine can check is above with its denominator. | Closure report and HOLD for [andres]'s gate B signature | The report states, for each of the two repositories: job list, test counts, assertion counts, pages scanned by axe, files scanned by each invariant. Any missing number is stated as missing, not omitted | T-806 |
