...
| T-106 theme approach | ⏸️ DEFERRED to unit 002 (see T-110) · **AMENDED 2026-08-25 by T-704 — append-only under rule 8; the deferral text to the left is the signed record and stands exactly as written.** ✅ **CLOSED by `9dc5722`** (`feat: the atomic swap — the template ships Ágora’s own theme`), the one commit that deleted `extra.drupal-site-template` from `composer.json`, added `drupal/agora_theme: ^1.0` to `require`, changed `install: - blank` to `- agora_theme` and pointed `system.theme.default` at it. ⚠️ **This closure is BOOKKEEPING, not engineering, and should never be read as work.** T-106 asked *“resolve the theme approach”*; **D-014=B resolved it on 2026-08-21**, four days before this row moved, and what `9dc5722` did was implement an answer that was already signed. T-704 wrote down that the pointer had stopped dangling. The engineering evidence lives with T-702 (implementation), T-703 (the deny-list coverage) and **T-705 (the clean-install proof)**, not here. | — | — |
